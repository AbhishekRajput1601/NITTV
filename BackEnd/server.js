...
io.on("connection", (socket) => {
    ...
    socket.on("start", () => {
        console.log(`User ${socket.id} started the stream`);
...
function matchUsers(socket) {
    console.log("in matchUser")
    // if (transitioningUsers.has(socket.id)) {
    //     return;
    // }
    console.log("transition dont have user")
    transitioningUsers.add(socket.id);

    availableUsers.delete(socket.id);
    
    console.log(`Attempting to match user: ${socket.id}`); 
    console.log("size :: ", availableUsers.size) 
    if (availableUsers.size > 0) {
        console.log(`Available users: ${Array.from(availableUsers).join(", ")}`);
        let otherUserId;
        ...
        if (otherUserId) {
            console.log(`Found match: ${socket.id} with ${otherUserId}`);
